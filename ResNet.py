...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks,toVit = False, include_to_kan:bool = False,num_classes=1000):
        ...
        self.layer = VisionTransformer(
            in_shape=(block.expansion*512,7,7),
            patch_size=(1,1),
            num_heads=16,
            d_MLP=1024,
            dropout=0.2,
            num_layers=12,
            classification=True
        ) if self.toVit else nn.AdaptiveAvgPool2d((1, 1))
        # self.avgpool = 
...
def ResNet18(num_classes,include_to_kan,toVit):
    return ResNet(BasicBlock, [2, 2, 2, 2],toVit,include_to_kan,num_classes)


def ResNet34(num_classes,include_to_kan,toVit):
    return ResNet(BasicBlock, [3, 4, 6, 3],toVit,include_to_kan,num_classes)


def ResNet50(num_classes,include_to_kan,toVit):
    return ResNet(Bottleneck, [3, 4, 6, 3],toVit,include_to_kan,num_classes)


def ResNet101(num_classes,include_to_kan,toVit):
    return ResNet(Bottleneck, [3, 4, 23, 3],toVit,include_to_kan,num_classes)


def ResNet152(num_classes,include_to_kan,toVit):
    return ResNet(Bottleneck, [3, 8, 36, 3],toVit,include_to_kan,num_classes)


def test():
    net = ResNet34(num_classes=10,include_to_kan=True,toVit=False).cuda()
    y = net(torch.randn(16, 1, 224, 224).cuda())
    print(y.size())
